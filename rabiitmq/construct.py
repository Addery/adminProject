...
def write_df2pcd(now: datetime.datetime, datas: List[DataFrame], path) -> None:
    ...
    for index, data in enumerate(datas):
        data.to_csv(os.path.join(save_path, f"{now.second}-{index}.csv"), index=False)
...
class Tunnel(object):
    ...
    def __init__(self, high, project_name, tunnel_name, working_face, structure, device_id, mileage,
                 data: PointCloudData = None):
        """
        TODO: 为隧道添加项目相关属性
        :param high: 隧道的实际高度
        :param project_name: 项目名称
        :param tunnel_name: 隧道名称
        :param working_face: 工作面
        :param structure: 结构物
...
class Segment(object):
    ...
    def subdivide(data: DataFrame) -> Region:
        ...
        # for geometry in geometries.get_pcds():
        #     if geometry != "null":
...
class Merge(object):
    ...
    @staticmethod
    def update_df(file_line, max_csv_path, max_csv_name, update_data, directory=TimeSeriesFile.DEFAULT_TIME_SERIES_SAVE_PATH):
        """
